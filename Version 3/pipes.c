 ...
 *
 * This version creates children that are clones of the parent process
 * The children send their reports by writing to an unnamed pipe.
 * The parent then reads the pipe and displays the matches.
 * =====================================================================================
 ...
 * gcc -Wall pipes.c -o pipes
 * ./pipes /Users/gabe/Desktop/Test/Data/Images /Users/gabe/Desktop/Test/Data/Patterns
 * -------------------------------------------------------------------------------------
 */
#include <stdio.h>
#include <stdlib.h>
...
void checkPattern(int imageHeight, int imageWidth, int patternHeight, int patternWidth,
...
int main(int argc, char *argv[]) {
    ...
    // our unnamed pipe for reading and writing from parent to child
    int myPipeFd[2];
    ...
    int totalMatchesArray[patternFileCount];
    memset(totalMatchesArray, 0, sizeof totalMatchesArray);
    // array with the # of pattern matches and the coordinates
    int searchResults[patternFileCount];
    // creating our pipe
    if(pipe(myPipeFd) == -1) {
        printf("%s\n", "There was an error making the pipe");
    }
    ...
    for(int imageFileId = 0; imageFileId < imageFileCount; imageFileId++) {
        ...
        if(pid == 0) {
            ...
            int countMatches[patternFileCount];
            //----------------------------------------------------------
            ...
            for(int patternFileId = 0; patternFileId < patternFileCount; patternFileId++) {
                ...
                checkPattern(imageHeight, imageWidth, patternHeight, patternWidth, imageArray, patternArray, searchResults);

                // make an array of all of the matches for each pattern file on one image
                countMatches[patternFileId] = searchResults[0];
            }
            // close reading end of the pipe
            close(myPipeFd[0]);
            printf("%s %d %s\n", "Child", getpid(), "writing to pipe");
            write(myPipeFd[1], &countMatches, sizeof(patternMatchArray));
            ...
            // parent process reads from the pipe and appends number to an array
            // It adds the next read results accordingly
            read(myPipeFd[0], &patternMatchArray, sizeof(totalMatchesArray));
            for(int i = 0; i < patternFileCount; i++) {
                totalMatchesArray[i] += patternMatchArray[i];
            }
    ...
    // we don't need the file path diectories anymore so free them from memory
    free(imageFilePath);
    ...
    for(int i = 0; i < patternFileCount; i++) {
        printf("%s %d %s %d %s", patternFileNames[i], i, "has", totalMatchesArray[i], "matches --> ");
        for(int j = 0; j < totalMatchesArray[i]; j++) {
            printf("%s ", "|");
        }
        printf("\n");
    }
